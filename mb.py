        ...
        with slim.arg_scope([slim.conv2d, slim.separable_conv2d], padding='SAME'):
            ...
            for i, conv_def in enumerate(conv_defs):
                ...
                if isinstance(conv_def, Conv):
                    ...
                    net = inverted_block(net, input_filters, depth(conv_def.depth), expand_ratio, conv_def.stride,
                                         end_point + '_0')
                    for index in range(1, conv_def.repeate):
                        suffix = '_' + str(index)
                        net = inverted_block(net, input_filters, depth(conv_def.depth), expand_ratio, 1,
                                             end_point + suffix)
...
def inference(images, num_classes=None, keep_probability=1.0, phase_train=False,
    ...
    with slim.arg_scope(arg_scope):
        return mobilenet_v2(images, num_classes=num_classes, dropout_keep_prob=keep_probability,
                            is_training=phase_train, reuse=reuse)
